...
async function createHistory() {
  ...
  var latestBlock
  // checkpoint
  await s3GetObjectPromise({ Bucket: 'stats.solace.fi.data', Key: 'output/swc/ethereum_v1.json'}).then(res => {
...
async function track_ethereum_v1() {
  return new Promise(async (resolve) => {
    console.log('start tracking swc ethereum v1')
    await prefetch()
    var ethereum_v1 = await createHistory()
    var res = JSON.stringify(ethereum_v1)
    await Promise.all([
      s3PutObjectPromise({ Bucket: 'stats.solace.fi.data', Key: 'output/swc/ethereum_v1.json', Body: res, ContentType: "application/json" }),
      s3PutObjectPromise({ Bucket: 'stats-cache.solace.fi', Key: 'swc/ethereum_v1.json', Body: res, ContentType: "application/json" })
    ])
    console.log('done tracking swc ethereum v1')
    resolve(ethereum_v1)
  })
}
exports.track_ethereum_v1 = track_ethereum_v1

//var res
//track_ethereum_v1().then(r=>{res=r;console.log(res)}).catch(console.error)
