// tracks policies in aurora over time
...
async function createHistory() {
  ...
  var latestBlock
  // checkpoint
  await s3GetObjectPromise({ Bucket: 'stats.solace.fi.data', Key: 'output/spi/aurora_v3.json'}).then(res => {
...
async function track_aurora_v3() {
  return new Promise(async (resolve) => {
    console.log('start tracking spi aurora v3')
    await prefetch()
    var aurora_v3 = await createHistory()
    var res = JSON.stringify(aurora_v3)
    await Promise.all([
      s3PutObjectPromise({ Bucket: 'stats.solace.fi.data', Key: 'output/spi/aurora_v3.json', Body: res, ContentType: "application/json" }),
      s3PutObjectPromise({ Bucket: 'stats-cache.solace.fi', Key: 'spi/aurora_v3.json', Body: res, ContentType: "application/json" })
    ])
    console.log('done tracking spi aurora v3')
    resolve(aurora_v3)
  })
}
exports.track_aurora_v3 = track_aurora_v3

//var res
//track_aurora_v3().then(r=>{res=r;console.log(res)}).catch(console.error)
