// tracks policies in polygon over time
...
const SPI_DEPLOY_BLOCK = 29804397
...
async function createHistory() {
  ...
  var latestBlock
  // checkpoint
  await s3GetObjectPromise({ Bucket: 'stats.solace.fi.data', Key: 'output/spi/polygon_v3.json'}).then(res => {
...
async function track_polygon_v3() {
  return new Promise(async (resolve) => {
    console.log('start tracking spi polygon v3')
    await prefetch()
    var polygon_v3 = await createHistory()
    var res = JSON.stringify(polygon_v3)
    await Promise.all([
      s3PutObjectPromise({ Bucket: 'stats.solace.fi.data', Key: 'output/spi/polygon_v3.json', Body: res, ContentType: "application/json" }),
      s3PutObjectPromise({ Bucket: 'stats-cache.solace.fi', Key: 'spi/polygon_v3.json', Body: res, ContentType: "application/json" })
    ])
    console.log('done tracking spi polygon v3')
    resolve(polygon_v3)
  })
}
exports.track_polygon_v3 = track_polygon_v3

//var res
//track_polygon_v3().then(r=>{res=r;console.log(res)}).catch(console.error)
