// tracks policies in fantom over time
...
const SPI_DEPLOY_BLOCK = 40957214
...
async function createHistory() {
  ...
  var latestBlock
  // checkpoint
  await s3GetObjectPromise({ Bucket: 'stats.solace.fi.data', Key: 'output/spi/fantom_v3.json'}).then(res => {
...
async function track_fantom_v3() {
  return new Promise(async (resolve) => {
    console.log('start tracking spi fantom v3')
    await prefetch()
    var fantom_v3 = await createHistory()
    var res = JSON.stringify(fantom_v3)
    await Promise.all([
      s3PutObjectPromise({ Bucket: 'stats.solace.fi.data', Key: 'output/spi/fantom_v3.json', Body: res, ContentType: "application/json" }),
      s3PutObjectPromise({ Bucket: 'stats-cache.solace.fi', Key: 'spi/fantom_v3.json', Body: res, ContentType: "application/json" })
    ])
    console.log('done tracking spi fantom v3')
    resolve(fantom_v3)
  })
}
exports.track_fantom_v3 = track_fantom_v3

//var res
//track_fantom_v3().then(r=>{res=r;console.log(res)}).catch(console.error)
