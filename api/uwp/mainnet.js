...
//const { withBackoffRetries, formatTimestamp, fetchBlock, fetchBalances, fetchBalanceOrZero, fetchSupplyOrZero, fetchScpPpsOrZero, fetchReservesOrZero, fetchUniswapV2PriceOrZero, fetchUniswapV3PriceOrZero } = require("./api/uwp/utils")
const { getProvider, s3GetObjectPromise, s3PutObjectPromise, snsPublishError } = require("./../utils")
const { withBackoffRetries, formatTimestamp, fetchBlock, fetchBalances, fetchBalanceOrZero, fetchSupplyOrZero, fetchScpPpsOrZero, fetchReservesOrZero, fetchUniswapV2PriceOrZero, fetchUniswapV3PriceOrZero } = require("./utils")
...
async function track_uwp_mainnet() {
  ...
  await Promise.all([
    s3PutObjectPromise({ Bucket: 'stats.solace.fi.data', Key: 'output/uwp_mainnet.csv', Body: csv }),
    s3PutObjectPromise({ Bucket: 'stats.solace.fi.data', Key: 'public/uwp_mainnet.csv', Body: csv })
  ])
